        ...
        <ul class="navbar-nav flex-row align-items-center ms-auto">
            ...
            <li class="nav-item navbar-dropdown dropdown-user dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                    ...
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('Can Access Setting')): ?>
                    <li>
                    ...
                    <li>
                        <a class="dropdown-item" href="javascript:void(0);" href="<?php echo e(route('logout')); ?>"
...
$('#load_notify_data').on('click', '.notification_click', function(event) {
    ...
    $.ajax({
        url: "<?php echo e(route('notification.show', ':id')); ?>".replace(':id', notificationId),
        method: 'GET',
...
function fetchNotificationCounts() {
    ...
    $(function() {
    // CSRF header (requires <meta name="csrf-token" content="<?php echo e(csrf_token()); ?>"> in your layout)
    $.ajaxSetup({
        headers: {
    ...
    $(document).on('click', '#mark-all-read', function(e) {
        e.preventDefault();
        $.ajax({
        url: '<?php echo e(route("notification.markAllRead")); ?>',
        method: 'POST'
        })
        .done(function() {
        $('.notificationCount').hide();
    ...
    $(document).on('click', '.notification_click', function(e) {
        ...
        if (wasUnread) {
        $.ajax({
            url: `<?php echo e(url('notifications')); ?>/${notifId}/read`,
            method: 'POST'
        })
        .done(function() {
    ...
    function getUser() {
        $.ajax({
            url: "<?php echo e(route('user.show')); ?>",
            type: "GET",
            success: function(response) {
...
</script><?php /**PATH C:\xampp\htdocs\RNT Automation\RNT Automation\resources\views/backend/includes/nav.blade.php ENDPATH**/ ?>