                        ...
                        <tr>
                            ...
                            <td>
                                <?php
                                    ...
                                    $currentUserReviewedCount = 0;

                                    // Count how many times the current user has approved in the review status
                                    foreach ($reviewStatus as $review) {
                                        if ($review['user_id'] == $currentUserId && $review['status'] == 'approved') {
                                            $currentUserReviewedCount++;
                                        }
                                    }

                                    // Count how many times the current user appears in the reviewers list
                                    $reviewers = explode(',', $note->toc_committee_member);
                                    $reviewers = array_map('trim', $reviewers);
                                    $currentUserOccurrences = count(array_filter($reviewers, function ($reviewer) use ($currentUserId) {
                                        return $reviewer == $currentUserId;
                                    }));

                                    // Determine if the user has reviewed at least once, but not all occurrences
                                    $currentUserPartiallyReviewed = $currentUserReviewedCount > 0 && $currentUserReviewedCount < $currentUserOccurrences;

                                    // Determine if the user has reviewed all occurrences
                                    $currentUserReviewedAll = $currentUserReviewedCount >= $currentUserOccurrences;
                                ?>

                                <?php if($currentUserReviewedAll): ?>
                                    <span class="badge bg-success">Reviewed</span>
                                <?php elseif($currentUserPartiallyReviewed): ?>
                                    <span class="badge bg-warning">Partially Reviewed</span>
                                <?php else: ?>
                            ...
                            <td>
                                ...
                                <a href="<?php echo e(route('toc.show.review',['id'=>$note->id])); ?>">
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\Procurement_final\resources\views/backend/toc_file/notes.blade.php ENDPATH**/ ?>