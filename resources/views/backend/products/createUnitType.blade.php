                ...
                <table id="Requisitions_Table" class="table">
                    ...
                    <tbody class="table-border-bottom-0" id="Categories-Table">
                        ...
                        <tr>
                            ...
                            <td>{{ $unitTypes->name }}</td>
                            <td>{{ $unitTypes->symbol }}</td>
                            <td>
                                @if ($unitTypes->is_active == 0)
                                <span class="badge bg-warning">Pending</span>
                                @elseif($unitTypes->is_active == 1)
...
$(document).ready(function() {
    ...
    $(document).on('click', '.editSubCategoryBtn', function() {
        ...
        $.ajax({
            url: "{{ route('unit-types.show', ':id') }}".replace(':id',
                subCategoryId),
            type: 'GET',
    ...
    $(document).on('click', '.deleteSubCategoryBtn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'DELETE',
                    url: "{{ route('unit-types.destroy', ':id') }}"
    ...
    $('#EditCategory-Submit').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('unit-types.update', ':id') }}".replace(':id',
                subCategoryId),
            type: "PUT",
            data: formData,
            success: function(response) {
