...
<div class="card">
                            ...
                            <div class="col-6">
                                <div class="mb-3">
                                    ...
                                    <select id="product-name" class="form-control" name="product_id" required>
                                        ...
                                        @foreach ($missingRequisitions as $product)
                                        <option value="{{ $product->product_name }}" data-spec="{{ $product->spec }}"
                                            data-unit_type="{{ $product->unit_type }}">{{ $product->product_name }}
                                        </option>
                                        @endforeach
                                    </select>
    ...
    <div class="container">
        <div class="header">
            <img src="{{ asset('Sheikh_Hasina_Medical_University,_Khulna_Logo.png') }}" alt="Logo">
        ...
        <div class="flex-container">
            <div class="flex-item">
                <p>
                    স্মারক নং:
                    <span id="requisitionNo">{{ $requisition->requisition_no }}</span>
            ...
            <div class="flex-item" style="text-align: right;">
                ...
                <p>তারিখ: {{ Carbon::parse($requisition->requisition_date)->format('d-F-Y') }}</p>
            </div>
        </div>
        <div class="content">
            <p>বরাবর,</p>
            ...
            <br>
            <p>চাহিদাকারীর নাম: <b>{{$requisition->user->name}}</b> চাহিদাকারীর পদবী:
                <b>{{$requisition->user->designation->designation}}</b>
            </p>
            <p>চাহিদার শ্রেণী:
            <div class="checkbox-group">
                <label>
                    <input type="checkbox" {{ $requisition->cc == 1 ? 'checked' : '' }} disabled> পণ্য ও সংশ্লিষ্ট সেবা
                </label>
                <label>
                    <input type="checkbox" {{ $requisition->cc == 2 ? 'checked' : '' }} disabled> কার্য ও ভৌত সেবা
                </label>
                <label>
                    <input type="checkbox" {{ $requisition->cc == 3 ? 'checked' : '' }} disabled> বৃদ্ধিবৃত্তিক সেবা
                </label>
                <label>
                    <input type="checkbox" {{ $requisition->cc == 4 ? 'checked' : '' }} disabled> অন্যান্য সেবা
                </label>
            </div>
            </p>
            <p>ক্রয় পরিকল্পনায় অন্তর্ভুক্ত কিনা:
            <div class="checkbox-group">
                <label><input type="checkbox" {{ $requisition->auth == 1 ? 'checked' : '' }} disabled>
                    অন্তর্ভুক্ত</label>
                <label><input type="checkbox" {{ $requisition->auth == 2 ? 'checked' : '' }} disabled> অন্তর্ভুক্ত করা
                    প্রয়োজন</label>
            </div>
            </p>
            <br>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                @endphp
                {{-- {{dd ($requisition)}} --}}
                @if($requisition->requisitionProducts->isEmpty())
                    @foreach ($requisition->missingRequisitions as $index => $requisition_product)
                    <tr>
                        <td>{{ $i++ }}</td>
                        <td>{{ $requisition_product->product_name }}</td>
                        <td>{!! $requisition_product->spec ?? 'N/A' !!}</td>
                        <td>{{ $requisition_product->unitType->name ??'N/A' }}</td>
                        <td>{{ $requisition_product->quantity }}</td>
                        <td>{{ strip_tags($requisition_product->note ?? 'N/A') }}</td>
                        @if ($index == 0) 
                            <!-- Add the new column with rowspan to span all rows and display raw HTML -->
                            <td rowspan="{{ count($requisition->missingRequisitions) }}">
                                {!! $requisition->remarks ?? 'No comment available' !!}
                            </td>
                        @endif
                    </tr>
                    @endforeach
                @else
                    @foreach ($requisition->requisitionProducts as $index => $requisition_product)
                    <tr>
                        <td>{{ $i++ }}</td>
                        <td>{{ $requisition_product->product->product_name }}</td>
                        <td>{!! $requisition_product->spec ?? 'N/A' !!}</td>
                        <td>{{ $requisition_product->unitType->name ??'N/A' }}</td>
                        <td>{{ $requisition_product->quantity }}</td>
                        <td>{{ strip_tags($requisition_product->remarks ?? 'N/A') }}</td>
                        @if ($index == 0) 
                            <!-- Add the new column with rowspan to span all rows and display raw HTML -->
                            <td rowspan="{{ count($requisition->requisitionProducts) }}">
                                {!! $requisition->remarks ?? 'No comment available' !!}
                            </td>
                        @endif
                    </tr>
        ...
        <div class="signature">
            @foreach ($requisition->requisitionSignatures as $index => $requisition_signature)
            @if ($index == 0)
            <div class="text-center">
                ...
                {{ Carbon::parse($requisition_signature->date)->format('d-F-Y') }}
                <br>
                <strong> {{ $requisition_signature->name }} </strong>
                <br>
                {{ $requisition_signature->designation }}
                <br>
                {{ $requisition_signature->department }}
            </div>
            @elseif ($index == 1)
            <div class="text-center">
                <img src="{{ asset('global_assets/user_images/signature/' . $requisition_signature->signature) }}"
        ...
        <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <!-- Right side container for the Send and Edit buttons -->
            <div>
                @if($requisition->status == 12 || $requisition->status == 13)
                <button type="button" class="btn btn-success editReturnRequisitionBtn" data-id="{{ $requisition->id }}" style="margin-right:5px;">
                    <i class="bx bx-edit me-1"></i> Edit
                </button>   
                @endif             
                <!-- Send Back button -->
                @if($requisition->status == 13)
                <button type="button" class="btn btn-success sendRequisitionBtn" data-id="{{ $requisition->id }}"
            ...
            $(document).ready(function() {
                ...
                $(document).on('click', '.editReturnRequisitionBtn', function() {
                    ...
                    location.href = "{{ route('requisition.return', ':id') }}".replace(':id', requisition_id);
                });
                ...
                $('#editRequisitionNo').on('click', function() {
                    ...
                    $('#requisitionNoInput').on('blur', function() {
                        ...
                        $.ajax({
                            url: "{{ route('requisitions.number.update', ':id') }}"
                                .replace(
                                    ':id',
                                    requisition_id),
                            type: 'PUT',
                            ...
                            error: function(xhr) {
                                ...
                                console.error('Error updating requisition number:',
                                    xhr
                                    .responseText);
                            }
                        });
                ...
                $(document).on('click', '.sendRequisitionBtn', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{ route('requisitions.sent', ':id') }}".replace(
                                    ':id',
                                    requisition_id),
                                type: 'POST',
                                headers: {
                ...
                if (requisition.user.auth_by == authUserId && status == 0) {
                    ...
                    <button type="button" class="btn btn-success acceptAuthrequisitionBtn"
                                        data-id="${requisition_id}" style="margin-right:5px;">Accepted</button>
                                    <button type="button" class="btn btn-danger rejectrequisitionBtn"
                                    data-id="${requisition_id}">Rejected</button>
                `);
                } else if (status == 1) {
                    $('#allocation').empty();
                    $('#allocation').append(`
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            response.data.forEach(function(productSubCategories) {
                                $('#product-sub-category, #edit-product-sub-category')
                                    .append(
                ...
                $(document).on('click', '.rejectrequisitionBtn', function() {
                    ...
                    var requisition_id = $(this).data('id');
                    $.ajax({
                        url: "{{ route('requisitions.reject', ':id') }}".replace(':id',
                            requisition_id),
                        type: 'GET',
                        success: function(response) {
                            if (response.status) {
                ...
                $(document).on('click', '.acceptrequisitionBtn', function() {
                    ...
                    $.ajax({
                        url: "{{ route('requisitions.edit', ':id') }}".replace(':id',
                            requisition_id),
                        type: 'GET',
                        success: function(response) {
                            if (response.status == true) {
                                ...
                                location.href = "{{ route('allocations.add') }}";
                            } else {
