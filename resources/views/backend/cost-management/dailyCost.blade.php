                ...
                <table id="Requisitions_Table" class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $dailyCost->cost_type }}</td>
                            <td>{{ $dailyCost->tnx_type }}</td>
                            <td>{{ $dailyCost->description }}</td>
                            <td class="text-end">{{ $dailyCost->amount }}</td>
                            <td>
                                <!-- Button to trigger the Edit Modal -->
                                <button type="button" class="btn btn-sm btn-primary edit-daily-cost" data-id="{{ $dailyCost->id }}" data-bs-toggle="modal" data-bs-target="#editDailyCostModal">
                                    Edit
                                </button>
                                <form action="{{ route('daily-costs.destroy', $dailyCost->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
...
<div class="modal fade" id="createDailyCostModal" tabindex="-1" aria-labelledby="createDailyCostModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="createDailyCostForm" action="{{ route('daily-costs.store') }}" method="POST">
                @csrf
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('.edit-daily-cost').on('click', function () {
            var id = $(this).data('id');
            var url = "{{ route('daily-costs.edit', ':id') }}";
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var updateUrl = "{{ route('daily-costs.update', ':id') }}";
        ...
        $.ajax({
            url: "{{ route('get-cost-types') }}", // Route to fetch cost types
            type: 'GET',
            success: function (response) {
