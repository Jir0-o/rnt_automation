    ...
    <div class="header" style="display: flex; align-items: center; justify-content: space-between; text-align: center;">
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-top: 0px;">
                        ...
                        <tr>
                            <td style="padding: 0px 0px 0px 5px;">
                                ...
                                @if ($requisition->order_type == 1)
                                    Cash Order
                                @elseif ($requisition->order_type == 2)
                                    Loan Order
                                @elseif ($requisition->order_type == 3)
                                    Sample Order
                                @endif
                            </td>
                        </tr>
        ...
        <table style="width: 100%; border-collapse: collapse; border: 1px solid black;">
            ...
            <tbody>
                ...
                @foreach ($requisition->orderProducts as $requisition_product)
                    @php 
...
<script>
    CKEDITOR.replace('newreturn-text');
        ...
        $.ajax({
            url: "{{ route('orders.return', ':id') }}".replace(':id', requisition_id), 
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}', 
                id: requisition_id,
        ...
        $(document).on('click', '.acceptOrderBtn', function() {
            ...
            var requisition_id = $(this).data('id');
            $.ajax({
                url: "{{ route('orders.accept', ':id') }}".replace(':id',
                    requisition_id),
                type: 'POST',
                headers: {
            ...
            var requisition_id = $(this).data('id');
            $.ajax({
                url: "{{ route('orders.reject', ':id') }}".replace(':id',
                    requisition_id),
                type: 'POST',
                headers: {
                    "X-CSRF-TOKEN": "{{ csrf_token() }}",
